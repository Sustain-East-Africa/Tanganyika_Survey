...
tanganyika <- read_excel("E:/Work/DASCOT/PROJECTS/R_Projects_Dascot/Tanganyika_Survey/tanganyika_survey_report/TNC Tanganyika_survey submissions.xlsx", sheet = "TNC Tanganyika - Main Questionn")
...
hh_items <- tanganyika %>% select(56:70)
hh_items <- hh_items %>%
...
## Household Assets ##
hh_assets <- tanganyika %>% select(87:93)
...
lh <- tanganyika %>% select(94, 110:154) #126:127, 141:145)
...
#--------- FISHING SECTION-------------------
fishing <- tanganyika %>% select(7, 273:290, 299)
fishing <- fishing %>% filter(`Is the household member who is a fisher currently present?`=="YES")
...
fishing_boat_options <- list("CANOE WITHOUT MOTOR", "CANOE BOAT", "LARGE BOAT WITHOUT ENGINE", "ENGINE BOAT", "DON’T FISH FROM BOAT","LARGE BOAT WITH ENGINE")

# Escaping special characters in the options
escaped_options <- escape_special_chars(fishing_boat_options)
...
fishing_gear_options <- list("LONG LINE", "GILL NETS", "RING NETS (PURSE-SEINE)", "LIFT NETS")

# Escaping special characters in the options
escaped_options <- escape_special_chars(fishing_gear_options)
...
fishing_livelihood <- tanganyika %>% select(7,273, 314:405)
fishing_livelihood <- fishing_livelihood %>% filter(`Is the household member who is a fisher currently present?`=="YES")
