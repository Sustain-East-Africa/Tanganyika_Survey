...
ggplot(treatment_data, aes(x = village, y = proportion * 100, group = treatment_method_dry, fill = treatment_method_dry)) +
  ...
  theme(legend.position = "bottom",  legend.direction = "horizontal",  legend.justification = "center", 
        axis.text.x = element_text(size = 8), strip.text = element_blank(), legend.box = 
...
```{r,  fig.cap='Main type of water treatment in the dry season', fig.align='center', out.width = '80%'}
...
ggplot(village_data, aes(x = drinking_water_dry, y = proportion * 100, fill = drinking_water_dry)) +
  geom_bar(stat = "identity", width = 0.8) +
  geom_errorbar(aes(ymax = pmin(proportion_upp, 1)*100, ymin = pmax(proportion_low, 0)*100),
                width = 0.2) +
  guides(fill = guide_legend(title = NULL)) +
  labs(x = "Household Items", y = "") +
  sea::scale_fill_sea_discrete() +
  sea::theme_sea() + 
  theme(legend.position = "top", legend.direction = "horizontal", legend.justification = "center", axis.text.x = element_text(size = 8))+
...
```{r,  fig.cap='Household item ownership', fig.align='center', out.width = '100%'}
