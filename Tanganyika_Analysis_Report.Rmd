...
Nearly half (45.5%) the population of the households surveyed was under the age of 15 (Figure 2). Most of the other half of the Mahale population is between 15 and 64 (52.2%); people aged 65 and older only make up 2.3% of the population.
...
The average household size in this survey was 5.2 (Table 2). This is smaller than the 6.7 members found in the 2011 Tuungane survey but the same as the 2010 DHS survey for rural Tanzania (NBS & ICF Macro, 2011). Female household heads make up 22% of the sample, which is higher than the findings from the 2011 Tuungane survey.
...
village_data <- bind_rows(village_data, total_data)
...
ggplot(village_data_combined, aes(x = reorder(WaterSource, -proportion), y = proportion * 100, fill = Season)) +
  ...
  scale_fill_manual(values = c("Dry" =  "#d2d2c8", "Wet" =  "#5b7a2e")) +
  labs(x = "", y = "", fill = "") +
  theme_minimal() +
...
# Combine the total_data with the original treatment_data
boat_type_data <- bind_rows(boat_type_data, total_data)
...
ggplot(boat_type_data, aes(x = village, y = proportion * 100, group = boat_type, fill = boat_type)) +
  ...
  guides(fill = guide_legend(title = NULL, nrow = 1, reverse = FALSE)) +
  labs(x = NULL, y = NULL) +
  scale_fill_manual(values = c("#3d5919", "#5b7a2e", "#a4b792", "#d2d2c8", "#d8e3cc", "#3c3c3c"),labels = c(
      "CANOE BOAT" = "Canoe", 
...
# Add a "Total" group to the data
total_data <- fishing_gear_design %>%
...
# Combine the total_data with the original treatment_data
fishing_gear_data <- bind_rows(fishing_gear_data, total_data)
...
# Add a "Total" group to the data
total_data <- processing_form_design %>%
...
# Combine the total_data with the original treatment_data
processing_form_data <- bind_rows(processing_form_data, total_data)
...
# Add a "Total" group to the data
total_data <- processing_equipment_design %>%
...
# Combine the total_data with the original treatment_data
processing_equipment_data <- bind_rows(processing_equipment_data, total_data)
